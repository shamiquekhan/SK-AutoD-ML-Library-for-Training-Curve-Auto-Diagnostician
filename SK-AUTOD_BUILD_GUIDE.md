...
- [ ] MLflow integration
- [ ] Jupyter magic command (`%sk-autod`)
- [ ] VS Code extension
...
**Deliverables:**
- `sk_autod/models.py` — `Finding`, `DiagnosisReport` dataclasses
- `sk_autod/preprocessor.py` — Input validation, alignment, smoothing, stats computation
...
**Deliverables:**
- `sk_autod/detectors/base.py` — BaseDetector abstract class
- `sk_autod/detectors/overfitting.py` — Overfitting detector
- `sk_autod/detectors/lr_high.py` — LR too high detector
- `sk_autod/detectors/early_stop.py` — Early stopping detector
...
**Deliverables:**
- `sk_autod/runner.py` — DiagnosticsRunner class
- Deduplication logic
- `sk_autod/__init__.py` — Public API
...
**Deliverables:**
- `sk_autod/formatters/text.py` — Human-readable text
- `sk_autod/formatters/json.py` — JSON output
...
- [ ] CLI integration: `sk-autod diagnose --train-loss [...] --val-loss [...]`
...
**Deliverables:**
- `sk_autod/cli.py` — Click/argparse CLI
...
- [ ] Build CLI using Click or argparse
  ```bash
  sk-autod diagnose --train-loss 2.3 1.9 1.4 --val-loss 2.4 2.0 1.8
  ```
- [ ] Accept input from:
...
- [ ] Verify: `pip install sk-autod` works
...
- [ ] Keras callback: `AutoDCallback()`
  ```python
  model.fit(..., callbacks=[AutoDCallback()])
  ```
- [ ] Notebook integration: `report.plot_with_annotations()`
- [ ] Weights & Biases logger:
  ```python
  wandb.log({"sk-autod": report.to_dict()})
  ```
...
- [ ] Can install from wheel: `pip install dist/sk_autod-0.1.0-py3-none-any.whl`

### Testing
- [ ] `pytest tests/ -v` passes 100%
- [ ] `pytest tests/ --cov=sk_autod` shows 80%+ coverage
- [ ] No warnings from mypy, ruff, black

### Release
- [ ] Tagged version: `git tag v0.1.0`
- [ ] Published to PyPI: `twine upload dist/*`
- [ ] Installable via pip: `pip install sk-autod==0.1.0`
...
```python
import pytest
from sk_autod.preprocessor import Preprocessor
...
This guide gives you everything needed to build SK-AutoD from scratch. The key to success:
